...
test.describe('确认向导 - 子步骤后退', () => {
  test('从步骤2点上一步应回到步骤1，而不是离开 confirm 页', async ({ page }) => {
    ...
    // 工程信息已经在种子数据里填好了，等自动跳转向导到步骤2（审查与评分合页）
    await expect(page.getByRole('button', { name: '核对无误，进入大纲策划' })).toBeVisible({ timeout: 10_000 });

    // 上一步 应该回到步骤1（按钮文案变回"下一步：审查与评分"）
    await page.getByRole('button', { name: '上一步' }).click();
    await expect(page.getByRole('button', { name: '下一步：审查与评分' })).toBeVisible();
