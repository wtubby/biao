...
3. 需要插入图表处，请直接嵌入以下格式的单行 JSON 占位符（切勿单独换行）：
   ...
   注意：不要输出 [GANTT_DATA] 施工进度横道图占位符——全局施工进度横道图会在文档末尾根据总工期统一生成一次，
   章节正文如需提及进度节点，用文字描述或 Markdown 表格即可，不要单独出图。
...
2. 方案/措施类：须含量化步骤与 **[参数] 数值+单位**；图表写入 embedded_charts，正文用 [[CHART:n]] 标记插入位置。
   不要在 embedded_charts 中输出 GANTT_DATA——全局横道图在文档末尾按总工期统一生成；进度节点用文字或 Markdown 表格即可。
...
不要在 embedded_charts 输出 GANTT_DATA（全局横道图在文档末尾按总工期统一生成）。"""
