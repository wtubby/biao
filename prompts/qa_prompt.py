...
请仅输出一个 JSON 对象，字段为 passed（布尔）、coverage_issues、faithfulness_issues、scope_issues、specificity_issues（均为字符串数组）。不要包含 Markdown 代码块或任何前后导言。"""

QA_MULTI_WINDOW_SYSTEM_PROMPT = """你是技术方案质检专家。检查正文是否完整回应评分项、品牌型号与规范标准号是否有检索素材/全局事实依据，以及是否超出当前章节范围（写了其他章节标题或内容）。
...
本次为长文多窗口抽检：输入含多个【窗口：标签】片段。请对每个窗口分别判断，仅基于该窗口正文，勿臆测窗口外内容。
请仅输出一个 JSON 对象，格式为：
{"segments":[{"label":"开头","passed":true,"coverage_issues":[],"faithfulness_issues":[],"scope_issues":[],"specificity_issues":[]}, ...]}
其中 label 必须与输入窗口标签一致；各 issues 均为字符串数组。不要包含 Markdown 代码块或任何前后导言。"""
...
def _qa_chapter_context_body(
    ...
    *,
    ...
    """本章质检上下文（随章节变化，置 project 稳定前缀之后）。"""
...
def _qa_multi_window_content_body(windows: list[tuple[str, str]]) -> str:
    parts = [f"【窗口：{label}】\n{body}" for label, body in windows]
    return "待质检正文窗口（请分别检查每一窗口）：\n\n" + "\n\n---\n\n".join(parts)


def build_qa_user_messages(
    content: str,
    bundle: dict,
    *,
    ...
    """分层 user 消息：全书稳定前缀与 Writer 一致，本章任务与正文置末。"""
    ...
    multi_windows = [w for w in (windows or []) if w]
    if len(multi_windows) > 1:
        labels = "、".join(label for label, _ in multi_windows)
        segment_line = (
            f"抽检模式：多窗口一次性质检（{labels}），"
            "请仅基于各窗口正文分别判断，勿臆测窗口外未给出的部分。\n"
        )
        content_msg = _qa_multi_window_content_body(multi_windows)
    elif segment_label:
        body = content
        segment_line = (
        ...
        content_msg = _qa_content_body(body)
    else:
    ...
    messages: list[str] = []
    project_prefix = build_cacheable_project_prefix(bundle)
    if project_prefix:
        messages.append(project_prefix)
    chapter_ctx = _qa_chapter_context_body(
        bundle,
        segment_line=segment_line,
        scope_block=scope_block,
        type_block=type_block,
    )
    if chapter_ctx.strip():
        messages.append(chapter_ctx.strip())
    messages.append(content_msg)
    return messages


def build_qa_chat_messages(
    content: str,
    bundle: dict,
    *,
    segment_label: str | None = None,
    ...
    multi = windows is not None and len(windows) > 1
    system = QA_MULTI_WINDOW_SYSTEM_PROMPT if multi else QA_SYSTEM_PROMPT
    messages: list[dict[str, str]] = [
        {"role": "system", "content": system},
    ]
    for part in build_qa_user_messages(
        content, bundle, segment_label=segment_label, windows=windows,
    ):
...
def build_qa_user_prompt(
    ...
    *,
    ...
    return _QA_MESSAGE_JOIN.join(
        build_qa_user_messages(
            content, bundle, segment_label=segment_label, windows=windows,
        )
    )
